...
namespace RemoteControlService.UniTests
{
    public class NightlyShutdownHistoryUpdaterTest
    {
        ...
        {
            var systemInformationMock = Substitute.For<ISystemInformation>();
            systemInformationMock.GetLastSystemShutdown().Returns(lastSystemShutdown);
            NightlyShutdownHistoryUpdater sut = new(systemInformationMock, shutdownHistoryStorage);
        ...
        {
            var systemInformationMock = Substitute.For<ISystemInformation>();
            systemInformationMock.GetLastSystemShutdown().Returns(new DateTime(2020, 12, 13, 3, 0, 0));
            NightlyShutdownHistoryUpdater sut = new(systemInformationMock, shutdownHistoryStorage);
            DateTime oldestShutdown = new(2020, 12, 8, 2, 0, 0);
        ...
        {
            var systemInformationMock = Substitute.For<ISystemInformation>();
            systemInformationMock.GetLastSystemShutdown().Returns(new DateTime(2020, 12, 12, 22, 0, 0));
            NightlyShutdownHistoryUpdater sut = new(systemInformationMock, shutdownHistoryStorage);
            DateTime oldestShutdown = new(2020, 12, 8, 2, 0, 0);
            shutdownHistoryStorage.Add(oldestShutdown);
            shutdownHistoryStorage.Add(new DateTime(2020, 12, 9, 1, 0, 0));
